...
class AdvertController extends Controller
{
    ...
    public function getAdverts($category_id = null, Request $request = null) {
        ...
        $adverts = Cache::remember('adverts_city_'.\Config::get('area')->id.'_category_'.$category_id.'_page_'.$request->page, $this->cache_minutes, function () use ($category_id) {
            ...
            if($category_id == null) {
                ...
                return $this->besplatnee->adverts()->getListForCityByHeading(\Config::get('area')->id, $category_id);
            }

        });

        //$adverts = $this->besplatnee->adverts()->_customEmptyPaginate(50);
    ...
    public function category(Request $request, $alias, $city, $sort) {
        ...
        $areaName = config('area')->country->getName() . ', ' . config('area')->getName();
        $this->breadcrumbs($areaName, route('city', ['city_alias' => config('area')->getInternationalName()]), $areaName);
        ...
        $this->title(implode(', ', $path) . ' в ' . \Config::get('area')->getInName());
    ...
    public function index(Request $request, $city_alias) {
        ...
        // foreach($this->besplatnee->headings()->getPathWithProperties($this->category, $this->filters) as $category) {
        //     $this->breadcrumbs(__($category['caption']), $category['url']);
        // }
        ...
        $this->header('Ещё БЕСПЛАТНЕЕ - интернет газета бесплатных объявлений и рекламы' . ' в ' . \Config::get('area')->getInName());
        $this->title('Ещё БЕСПЛАТНЕЕ - интернет газета бесплатных объявлений и рекламы' . ' в ' . \Config::get('area')->getInName());
        $this->description('Ещё БЕСПЛАТНЕЕ - интернет газета бесплатных объявлений и рекламы' . ' в ' . \Config::get('area')->getInName());
        $this->keywords(\Str::lower(implode(', ', $path)));
    ...
    public function advert(Request $request, $alias_local, $city_local, $alias_international, $city_international, $id) {
        ...
        $this->version = 'b';
        $advertTitle = \Str::words($advert->content, 5, '') . ' в ' . \Config::get('area')->getInName();
        $this->header($advert->name . ' в ' . \Config::get('area')->getInName());
        ...
        $this->keywords(implode(',', $keywords) . ',' . str_replace(' ', ',', \Str::words($advert->content, 5, '')) . ',в ' . \Config::get('area')->getInName());
    ...
    public function create() {
        $this->header(__('adverts.create.seo.header', ['city' => \Config::get('area')->getInName()]));
        $this->title(__('adverts.create.seo.title', ['city' => \Config::get('area')->getInName()]));
        $this->description(__('adverts.create.seo.description', ['city' => \Config::get('area')->getInName()]));
        $this->keywords(__('adverts.create.seo.keywords', ['city' => \Config::get('area')->getInName()]));
    ...
    public function save(Request $request) {
        $this->header(__('adverts.save.seo.header', ['city' => \Config::get('area')->getInName()]));
        $this->title(__('adverts.save.seo.title', ['city' => \Config::get('area')->getInName()]));
        $this->description(__('adverts.save.seo.description', ['city' => \Config::get('area')->getInName()]));
        $this->keywords(__('adverts.save.seo.keywords', ['city' => \Config::get('area')->getInName()]));
    ...
    public function detail(Request $request, $id) {
        $this->header(__('adverts.detail.seo.header', ['city' => \Config::get('area')->getInName()]));
        $this->title(__('adverts.detail.seo.title', ['city' => \Config::get('area')->getInName()]));
        $this->description(__('adverts.detail.seo.description', ['city' => \Config::get('area')->getInName()]));
        $this->keywords(__('adverts.detail.seo.keywords', ['city' => \Config::get('area')->getInName()]));
    ...
    public function preview(Request $request, $id) {
        $this->header(__('adverts.preview.seo.header', ['city' => \Config::get('area')->getInName()]));
        $this->title(__('adverts.preview.seo.title', ['city' => \Config::get('area')->getInName()]));
        $this->description(__('adverts.preview.seo.description', ['city' => \Config::get('area')->getInName()]));
        $this->keywords(__('adverts.preview.seo.keywords', ['city' => \Config::get('area')->getInName()]));
    ...
    public function edit(Request $request, $id) {
        $this->header(__('adverts.edit.seo.header', ['city' => \Config::get('area')->getInName()]));
        $this->title(__('adverts.edit.seo.title', ['city' => \Config::get('area')->getInName()]));
        $this->description(__('adverts.edit.seo.description', ['city' => \Config::get('area')->getInName()]));
        $this->keywords(__('adverts.edit.seo.keywords', ['city' => \Config::get('area')->getInName()]));
    ...
    public function success(Request $request, $id) {
        $this->header(__('adverts.success.seo.header', ['city' => \Config::get('area')->getInName()]));
        $this->title(__('adverts.success.seo.title', ['city' => \Config::get('area')->getInName()]));
        $this->description(__('adverts.success.seo.description', ['city' => \Config::get('area')->getInName()]));
        $this->keywords(__('adverts.success.seo.keywords', ['city' => \Config::get('area')->getInName()]));
    ...
    public function search(Request $request) {
        ...
        switch($search_area_id) {
            case 'world':
                    $cities_q = ['0' => 'По всему миру'];
                break;
            case 'country':
                    $cities = \App\City::where('active', true)->where('country_id', \Config::get('area')->country->id)->pluck('id');

                    $area = \App\Country::find(\Config::get('area')->country->id);

                    $cities_q = [$area->id => $area->getName()];
                break;
            case 'region':
                    $cities = \App\City::where('active', true)->where('region_id', \Config::get('area')->region->id)->pluck('id');
                    
                    $area = \App\Region::find(\Config::get('area')->region->id);

                    $cities_q = [$area->id => $area->getName()];
                break;
            default:
                    $cities = [$search_area_id];
                    
                    $area = \App\City::find($cities[0]);
        ...
        $sended_data = ['alias' => $query, 
                        'alias_international' => str_replace('-', '_', app('slug')->make($query)), 
                    ];
        $route_name = 'search_redirect_null';
        $city_alias = '';

        if (!empty($cities)) {
            $city_alias = \Config::get('area')->getInternationalName();
            $city_name = mb_convert_case(\Config::get('area')->getName(), MB_CASE_LOWER, "UTF-8");

            $sended_data['city'] = $city_alias;
            $sended_data['city_name'] = $city_name;
            $sended_data['city_name_international'] = app('slug')->make($city_name);
        ...
        /*foreach($request->input('properties', []) as $name => $value) {
            if(!empty($value)) {
        ...
        $this->adverts = $this->besplatnee->adverts()->getList([
            'limit' => 50,
            'paginate' => true,
            'headings' => $headings,
            'cities' => $cities,
        ...
        if($query != '') {
        	$searched_ids = \App\Advert::where('name', 'like', "%{$query}%")->orWhere('content', 'like', "%{$query}%")->pluck('id');
        	$this->adverts = $this->adverts->whereIn('id', $searched_ids);
        }
    ...
    public function searchRedirectNull (Request $request, $alias, $alias_international) {
        ...
        return $this->searchRedirect($request, null, $alias, 'мире', $alias_international, 'mire');
    }

    public function searchRedirect(Request $request, $region, $alias, $city_name, $alias_international, $city_name_international) {
        $this->header('Результаты поиска');
        $this->title('Поиск '.str_replace('_', ' ', $alias).' в городе '.\Str::ucfirst($city_name));
        $this->description('Поиск '.str_replace('_', ' ', $alias).' в городе '.\Str::ucfirst($city_name));
        $this->keywords('поиск, '.str_replace('_', ', ', $alias).', в городе, '.\Str::ucfirst($city_name));

        $this->version = 'b';

        $this->buildSorts();

        $search_area_id = session('search_area_id') ? [session('search_area_id')] : [\Config::get('area')->id];
        ...
        $advert_search_query_city->firstOrCreate([
            ...
            'city_id'           =>  $search_area_id[0]
        ]);

        $this->adverts = Cache::remember('query_'.$query_data->id.'_city_'.$search_area_id[0].'_page_'.$request->page, $this->cache_minutes, function () use ($headings, $search_area_id, $alias) {
        ...
        if ($this->adverts->count() == 0) {
            $empty = Cache::remember('empty_search_results', $this->cache_minutes, function () use ($search_area_id) {
        ...
        $advert_ids = Cache::remember('query_'.$query_data->id.'_city_'.$search_area_id[0].'_ids', $this->cache_minutes, function () use ($headings, $search_area_id, $alias) {
