...
<div class="create-ad-wrapper ad-post">
    ...
    <form action="{{ request()->input('id') ? route('admin.edit', ['model' => 'geoobjects', 'id' => $gobject->id]) : route('admin.create', ['model' => 'geoobjects']) }}" id="create-ad" method="post">
        ...
        @if (isset(request()->parent))
            ...
            <input type="hidden" name="parent_id" value="{{ request()->input('parent') }}">

            <?php
                $chain = $parent->makeChain()->implode('name', ' / ');
            ...
            @include('elements.forms.wrapper', [
                'name' => '',
                'color' => 'black',
                'title' => 'Будет создан геообъект в разделе '.(!empty($chain) ? $chain.' / ' : '').$parent->name,
                'input' => 'hidden',
                'help' => '',
                'desc' => '',
                'id' => 'gobject_parent',
