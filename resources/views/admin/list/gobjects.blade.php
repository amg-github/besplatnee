...
<div class="admin-filters row">
    <div class="col-xs-2">
        <div class="breadcrumbs-wrapper">
            <ul class="breadcrumbs">
                @if(app('request')->parent)
                    @forelse($items->first()->makeChain() as $chain)
                        {{ $chain->name }}
    ...
    @if(!isset(app('request')->name))
        <form action="{{ route('admin.create', ['model' => $model]) }}">
            <input type="hidden" name="parent" value="{{ app('request')->parent }}">
            <button class="col-xs-2 ymap-search">Создать геообъект в текущем разделе</button>
        </form>
    @endif

    <form action="{{ route('admin.list', ['model' => $model]) }}">
        <input class="col-xs-3" style="height: 25px; padding: 0px 12px; margin: 0 10px; font-size: 14px" type="text" placeholder="Название" name="name" value="{{ app('request')->name }}">
        <button class="col-xs-3 ymap-search">Поиск</button>
    </form>
